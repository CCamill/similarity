    ...
    for function_dict in data:
        ...
        for block in function['function_blocks']:
            for inst in block['block_insts']:
                ...
                instruction = nomal_const(instruction, function_consts, consts_norm_consts_map)
                ...
                if len(inst['operand_list']) > 0:
                    for operand in inst['operand_list']:
                        ...
                        inst['operand_list'][operand_index] = nomal_const(operand, function_consts, consts_norm_consts_map)

                if inst.get("condition"):
                    condition = inst['condition']
                    if condition:
                        if "call" in condition or "invoke" in condition:
                            inst['condition'] = nomal_const(condition, function_consts, consts_norm_consts_map)
                
                if inst.get("return_value"):
                    return_value = inst['return_value']
                    if return_value:
                        if "call" in return_value or "invoke" in return_value:
                            inst['return_value'] = nomal_const(return_value, function_consts, consts_norm_consts_map)
    return 1
