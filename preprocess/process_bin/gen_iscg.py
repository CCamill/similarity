...
def process_summary_file(summary_path, global_path, struct_path, iscg_path):
    ...
    try:
        for function_dict in summary_data:
            ...
            function_inst_num = function['last_inst_id'] + 1
                ...
                for index,inst_info in enumerate(block_info['block_insts']):
                    ...
                    if inst_info['opcode'] == 'store':
                        ...
                        if operand_1.split()[-1] in function_params and operand_2 in function_instructions:
                            ...
                            operand_id = function_inst_num + const_node_count
                            G.add_node(str(operand_id),value = operand_1.split()[-1])
                            const_node_count += 1
                            if operand_2 in function_instructions:
                                G.add_edge(operand_id, inst_instID_dict[operand_2], edge_type='DataStore')
                            if operand_2.split()[-1] in function_params:
                                G.add_edge(operand_id, operand_2.split()[-1], edge_type='DataStore')
                        else:
                            pass
                        continue
                    ...
                    for operand in inst_info['operand_list']:
                        ...
                        if operand in global_list:
                            ...
                            if type != 'string' and type != 'struct':
                                ...
                                attr_dict = {"define":define, "var_name":var_name, "value":value, "linkage":linkage, "type":type, "value_lens":value_lens, "is_global":'true'}
