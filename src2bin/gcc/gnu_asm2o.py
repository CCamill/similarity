...
GCC_ASM = r'/home/lab314/cjw/similarity/datasets/bin/gcc_asm'
...
def main():
    for proj in os.listdir(GCC_ASM):
        log_path = os.path.join(GCC_O, proj, 'log.txt')
        if os.path.exists(log_path):
            continue
        asm_dir = os.path.join(GCC_ASM, proj,  'bin_files')
        if not os.path.exists(asm_dir):
            continue
        print(f"[+] Processing {proj}")
        for opti in os.listdir(asm_dir):
            opti_dir = os.path.join(asm_dir, opti)
            o_opti_dir = os.path.join(GCC_O, proj, opti)
            ll_opti_dir = os.path.join(GCC_LL, proj, opti)
